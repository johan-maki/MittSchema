...
export const AddProfileDialog = ({
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div>
        <label className="text-sm font-medium text-[#1A1F2C] dark:text-gray-300">Erfarenhetsnivå (år)</label>
        <Input
          type="number"
          min="0"
          max="50"
          value={newProfile.experience_level}
          onChange={e => setNewProfile(prev => ({ ...prev, experience_level: parseInt(e.target.value) || 1 }))}
          className="dark:bg-gray-800 dark:text-white dark:border-gray-700"
        />
      </div>
