...
export const ScheduleConfigModal: React.FC<ScheduleConfigModalProps> = ({
  ...
  const [config, setConfig] = useState<ScheduleConfig>({
    ...
    weekendPenalty: currentConfig.weekendPenalty || 1000,
  });
        ...
        <div className="p-6 space-y-8 overflow-y-auto flex-1">
          ...
          <div className="space-y-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <label className="text-lg font-semibold text-gray-700">
                  Rättvisa för helgpass
                </label>
                <p className="text-sm text-gray-500">Högre värde = mer rättvis fördelning av helgpass</p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
              <input
                ...
                max="2000"
                step="100"
                value={config.weekendPenalty}
                onChange={(e) => handleInputChange('weekendPenalty', parseInt(e.target.value))}
                className="flex-1 h-3 bg-gray-200 rounded-lg appearance-none cursor-pointer slider"
              />
              <div className="bg-purple-100 text-purple-800 px-4 py-2 rounded-lg text-lg font-bold min-w-[5rem] text-center">
                {config.weekendPenalty}
              </div>
            </div>
            <div className="text-xs text-gray-400 flex space-x-4">
              <span>Låg (100) = Snabbare</span>
              <span>Hög (2000) = Mer rättvist</span>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div className="bg-white p-3 rounded-lg">
                <div className="text-gray-500">Helgpass-rättvisa</div>
                <div className="text-xl font-bold text-purple-600">{config.weekendPenalty}</div>
              </div>
              <div className="bg-white p-3 rounded-lg">
